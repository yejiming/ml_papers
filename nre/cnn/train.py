...
from nre.cnn.text_cnn import TextCNN
...
# Data loading params
tf.flags.DEFINE_string("train_dir", "../data/SemEval2010_task8_all_data/SemEval2010_task8_training/TRAIN_FILE.TXT", "Path of train data")
...
def train():
    with tf.device("/cpu:0"):
        x_text, pos1, pos2, y = data_utils.load_data_and_labels(FLAGS.train_dir)
    ...
    with tf.Graph().as_default():
        with tf.Session() as sess:
            cnn = TextCNN(sequence_length=x_train.shape[2],
            ...
            for batch in batches:
                ...
                if step % FLAGS.evaluate_every == 0:
                    ...
                    loss, accuracy, predictions = sess.run([cnn.loss, cnn.accuracy, cnn.predictions], feed_dict)

                    time_str = datetime.datetime.now().isoformat()
                    print("{}: step {}, loss {:g}, acc {:g}".format(time_str, step, loss, accuracy))
                    print("(2*9+1)-Way Macro-Average F1 Score (excluding Other): {:g}\n"
                          .format(f1_score(np.argmax(y_dev, axis=1), predictions, labels=np.array(range(1, 19)), average="macro")))
