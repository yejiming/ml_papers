...
class KnowledgeGraph:
    ...
    def next_valid_batch(self, batch_size):
        rand_idx = np.random.permutation(self.n_validation_triple)
        start = 0
        while start < self.n_validation_triple:
            end = min(start + batch_size, self.n_validation_triple)
            yield [self.validation_triples[i] for i in rand_idx[start:end]]
            start = end
    ...
    def generate_training_batch(self, raw_batch):
        batch_pos = raw_batch
        batch_neg = []
