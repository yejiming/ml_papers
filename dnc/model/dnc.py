...
from dnc.model.memory import Memory
...
class DNC:
    ...
    def build_graph(self):
        ...
        """
        self.unpacked_input_data = utils.unpack_into_tensorarray(self.input_data, 1, self.sequence_length)
        ...
        with tf.control_dependencies(dependencies):
            self.packed_output = utils.pack_into_tensor(final_results[2], axis=1)
            self.packed_memory_view = {
                "free_gates": utils.pack_into_tensor(final_results[3], axis=1),
                "allocation_gates": utils.pack_into_tensor(final_results[4], axis=1),
                "write_gates": utils.pack_into_tensor(final_results[5], axis=1),
                "read_weightings": utils.pack_into_tensor(final_results[6], axis=1),
                "write_weightings": utils.pack_into_tensor(final_results[7], axis=1),
                "usage_vectors": utils.pack_into_tensor(final_results[8], axis=1)
            }
