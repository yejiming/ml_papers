    ...
    def test_pairwise_add(self):
        ...
        with graph.as_default():
            with tf.Session(graph=graph) as session:
                ...
                U_op = utils.pairwise_add(u)
                UV_op = utils.pairwise_add(u, v)
    ...
    def test_pairwise_add_with_batch(self):
        ...
        with graph.as_default():
            with tf.Session(graph=graph) as session:
                ...
                U_op = utils.pairwise_add(u, is_batch=True)
                UV_op = utils.pairwise_add(u, v, is_batch=True)
    ...
    def test_unpack_into_tensorarray(self):
         ...
         with graph.as_default():
             with tf.Session(graph=graph) as session:
                 ...
                 ta = utils.unpack_into_tensorarray(T, axis=1)
    ...
    def test_pack_into_tensor(self):
         ...
         with graph.as_default():
             with tf.Session(graph=graph) as session:
                ...
                T = tf.random_normal([5, 10, 7, 7])
                ta = utils.unpack_into_tensorarray(T, axis=1)
                pT = utils.pack_into_tensor(ta, axis=1)
