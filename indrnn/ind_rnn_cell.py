    ...
    def build(self, inputs_shape):
        ...
        if self._input_initializer is None:
            self._input_initializer = init_ops.random_normal_initializer(mean=0.0,
                                                                   stddev=0.001)
        self._input_kernel = self.add_variable("input_kernel", shape=[input_depth, self._num_units],
        ...
        self._recurrent_kernel = self.add_variable("recurrent_kernel", shape=[self._num_units],
        ...
        if self._recurrent_min_abs:
            ...
            self._recurrent_kernel = math_ops.multiply(math_ops.sign(self._recurrent_kernel), min_abs_kernel)
